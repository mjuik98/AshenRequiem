...
- 메타 씬(`SettingsScene`, `MetaShopScene`, `CodexScene`)은 세션/콘텐츠 규칙을 직접 조합하지 않고 scene-facing application service를 통해 호출한다. `MetaShopScene`은 `src/app/meta/metaShopSceneApplicationService.js`, `CodexScene`은 `src/app/meta/codexSceneApplicationService.js`, settings data-panel flow는 `src/app/session/settingsSceneApplicationService.js`가 각각 소유한다.
...
- `SettingsScene`는 더 이상 `SettingsView` callback을 inline으로 조립하지 않는다. `src/app/session/settingsSceneApplicationService.js`의 `createSettingsSceneHandlers()`가 runtime deps factory와 title 복귀 callback을 주입받아 save/import/reset/restore/export/inspect handler를 한 곳에서 조립한다.
- codex scene 진입 전 세션 보정 실소유권은 `src/app/session/codexSessionStateService.js`로 이동했다. `src/app/meta/codexApplicationService.js`는 session owner service를 호출해 준비된 `session`과 `gameData`만 만든다. `src/app/meta/codexSceneApplicationService.js`는 이 low-level prepare helper를 감싸 `CodexView`가 바로 소비할 scene-facing payload를 조립한다.
...
- `MetaShopScene`의 초기 view payload와 purchase 후 refresh 판단은 `src/app/meta/metaShopSceneApplicationService.js`가 소유한다. scene는 injected scene factory를 통한 전환과 `MetaShopView` lifecycle만 유지하고, low-level purchase 결과 해석은 더 이상 직접 하지 않는다.
...
- 투사체/이펙트 렌더러는 `src/renderer/sprites/vfxSpriteManifest.js`와 `src/renderer/sprites/vfxSpriteRuntime.js`를 통해 raster sprite source를 lazy-load한다. combat VFX 계약은 `projectileVisualId`, `impactEffectVisualId`, `effectVisualId`를 기준으로 표준화되며, shared atlas(`projectiles`, `effects`)와 standalone 4x4 sheet source를 함께 지원한다. `magic_bolt` / `arcane_nova` / `piercing_spear` / `astral_pike` / `holy_bolt` / `holy_bolt_upgrade` / `ice_bolt` / `ice_bolt_upgrade`를 포함한 cataloged combat visuals는 low-quality에서도 축소된 glow/stretch를 적용한 sprite 경로를 유지하고, vector draw registry는 uncataloged combat visuals와 utility/debug effect(`damageText`, `levelFlash`)에만 남는다.
...
- `SettingsScene`는 더 이상 `src/app/meta/settingsApplicationService.js`를 직접 보지 않고, scene-facing entrypoint인 `src/app/session/settingsSceneApplicationService.js`를 통해 settings snapshot/save/import/reset use-case를 조립한다. scene는 runtime dependency assembly와 navigation만 남기고, import preview/result payload shaping과 오류 메시지 매핑은 session-owned scene service가 맡는다. 기존 `src/app/meta/settingsApplicationService.js`는 thin compatibility facade만 유지한다.
- 시작 무기 후보/선택 해석 SSOT는 `src/domain/meta/loadout/startLoadoutDomain.js`다. 내부 책임은 `startLoadoutCatalog.js`, `startLoadoutUnlocks.js`, `startLoadoutSelection.js`, `startLoadoutPlayerStart.js`, `startLoadoutPresentation.js`로 분리됐다. 실제 플레이어 스폰 패키지 조립은 `src/app/play/playerSpawnApplicationService.js`의 `resolvePlayerSpawnState()`가 담당한다.
...
- 타이틀 시작 무기 선택 UI의 scene-facing payload와 callback assembly는 `src/app/title/titleLoadoutSceneApplicationService.js`가 소유한다. low-level 저장과 PlayScene 생성 orchestration은 `src/app/title/titleLoadoutApplicationService.js`가 계속 담당하고, query payload shaping은 `titleLoadoutQueryService.js`가 맡는다.
...
- `TitleScene`는 `titleSceneRuntimeState.js`가 만든 runtime state 객체를 소유한다. `titleSceneRuntime`, `titleSceneNavigation`, `titleSceneInput`, `titleLoadoutFlow`는 이 명시 state를 통해 DOM/background/loadout/nav/listener를 읽고 쓴다. 이 중 `titleLoadoutFlow`는 이제 view loader와 open-state 판별만 유지하고, `StartLoadoutView`에 넘길 payload/callback assembly는 `titleLoadoutSceneApplicationService`에 위임한다. 타이틀 shell DOM 자체는 `titleSceneShell.js`가 재사용 가능한 root + cached refs(`canvas`, `flash`, `live`)로 유지한다.
- 플레이어 영구 업그레이드 카탈로그는 `src/data/permanentUpgradeCatalog.js`가, 실제 적용 로직은 `src/domain/play/progression/permanentUpgradeApplicator.js`가 소유한다. `src/data/permanentUpgradeApplicator.js`와 `permanentUpgradeData.js`는 테스트/기존 import 호환용 facade만 유지하고, app/play 내부 caller는 play domain owner를 직접 본다.
...
- 세션 입력 옵션(`options.keyBindings`)은 `src/input/keyBindings.js`가 정규화하고, `KeyboardAdapter`와 `TitleScene` confirm 입력이 같은 binding 규칙을 공유한다. browser-specific `KeyboardAdapter` 구현은 `src/platform/browser/input/KeyboardAdapter.js`가 소유하고, `src/input/KeyboardAdapter.js`는 호환 facade만 유지한다. adapter는 `createGameInput()`이 주입한 runtime host에만 listener를 붙이고, settings save/import/reset/restore는 `input.configureKeyBindings()`를 통해 즉시 반영된다.
- 입력 runtime은 `KeyboardAdapter` 외에 capability-gated `GamepadAdapter`를 함께 등록한다. `GamepadAdapter`와 `TouchAdapter`의 실제 owner도 `src/platform/browser/input/*`로 이동했고, 구 경로 `src/input/*Adapter.js`는 테스트/기존 import 호환용 wrapper만 유지한다. gamepad의 left stick/d-pad와 confirm/pause/debug 버튼은 같은 `InputState.actions` contract로 합류한다.
- touch 입력은 `TouchAdapter`가 좌측 virtual joystick HUD와 우측 pause 버튼 overlay를 함께 제공한다. HUD DOM lifecycle과 knob sync는 `src/platform/browser/input/touchHudRuntime.js`가 소유하고, adapter는 입력 정규화와 action bridge만 담당한다. 이동은 기존 `moveX/moveY`, pause는 `actions.add('pause')` 경로로 합류한다.
...
- `sessionPersistenceService`, `sessionSnapshotQueryService`, `sessionSnapshotCommandService`는 더 이상 browser repository adapter를 직접 import하지 않는다. app/session seam인 `src/app/session/sessionRepositoryPort.js`가 browser-backed default 구현을 조립하고, 개별 service는 이 local port만 소비한다. `sessionFacade`는 계속 thin re-export만 유지한다.
...
- projectile/effect visual-ID contract with shared atlas + standalone sheet support, animated magic_bolt / arcane_nova / fire_bolt / holy_bolt / ice_bolt sequences, sprite-authoritative cataloged combat VFX, and utility/debug-only vector fallback: `vfxSpriteManifest`, `vfxSpriteRuntime`, `drawBehaviorRegistry`, `drawEffectRegistry`, `public/assets/vfx/*.png`
...
- 메타/진입 application service 계층: `settingsApplicationService`, `metaShopApplicationService`, `codexApplicationService`, `titleLoadoutApplicationService`, `titleLoadoutSceneApplicationService`, `startRunApplicationService`, `playResultApplicationService`
