...
function queueBurst(world, enemy, phaseAction) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    world.queues.spawnQueue.push(spawnEffect({
      ...
      y: enemy.y + Math.sin(angle) * ringRadius,
      config: {
        effectVisualId: phaseAction.effectVisualId ?? phaseAction.effectType ?? 'burst',
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    world.queues.spawnQueue.push(spawnProjectile({
      ...
      config: {
        ...
        projectileVisualId: phaseAction.projectileVisualId ?? enemy.projectileConfig.projectileVisualId ?? null,
        impactEffectType: phaseAction.impactEffectType ?? enemy.projectileConfig.impactEffectType ?? null,
        impactEffectVisualId: phaseAction.impactEffectVisualId ?? enemy.projectileConfig.impactEffectVisualId ?? enemy.projectileConfig.impactEffectType ?? null,
        ownerId: enemy.id,
        ownerType: 'enemy',
...
function queueReposition(world, enemy, phaseAction) {
  ...
  world.queues.spawnQueue.push(spawnEffect({
    ...
    y: enemy.y,
    config: {
      effectVisualId: phaseAction.effectVisualId ?? 'burst',
