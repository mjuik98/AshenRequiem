...
function sortCards(cards, activeSort) {
  const list = [...cards];
  if (activeSort === 'price') {
    return list.sort((left, right) => (left.cost - right.cost) || (right.recommendedWeight - left.recommendedWeight) || (left.originalIndex - right.originalIndex));
  }
...
export function buildMetaShopViewModel(session, {
  ...
  const cards = upgradeData.map((upgrade, originalIndex) => {
