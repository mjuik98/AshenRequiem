...
await test('title loadout application service는 option/commit/result helper module로 분해된다', async () => {
  let titleRunOptions;
  let titleRunSelectionCommit;
  let titleRunResult;

  try {
    titleRunOptions = await import('../src/app/title/titleRunOptions.js');
    titleRunSelectionCommit = await import('../src/app/title/titleRunSelectionCommit.js');
    titleRunResult = await import('../src/app/title/titleRunResult.js');
  } catch (error) {
    throw new Error(`title loadout helper import 실패: ${error.message}`);
  }

  assert.equal(typeof titleRunOptions.normalizeTitleRunOptions, 'function', 'titleRunOptions normalize helper가 없음');
  assert.equal(typeof titleRunOptions.extractTitleRunDeps, 'function', 'titleRunOptions deps helper가 없음');
  assert.equal(typeof titleRunSelectionCommit.commitTitleStartWeaponSelection, 'function', 'titleRunSelectionCommit helper가 없음');
  assert.equal(typeof titleRunResult.buildTitleRunResult, 'function', 'titleRunResult helper가 없음');
  assert.match(readProjectSource('../src/app/title/titleLoadoutApplicationService.js'), /from '\.\/titleRunOptions\.js'/, 'titleLoadoutApplicationService가 option helper를 사용하지 않음');
  assert.match(readProjectSource('../src/app/title/titleLoadoutApplicationService.js'), /from '\.\/titleRunSelectionCommit\.js'/, 'titleLoadoutApplicationService가 commit helper를 사용하지 않음');
  assert.match(readProjectSource('../src/app/title/titleLoadoutApplicationService.js'), /from '\.\/titleRunResult\.js'/, 'titleLoadoutApplicationService가 result helper를 사용하지 않음');
});
...
await test('UpgradeSystem은 choice/fallback/apply helper 모듈로 분리된다', async () => {
  ...
  try {
    upgradeChoicePool = await import('../src/domain/play/progression/upgradeChoicePool.js');
    upgradeFallbackChoices = await import('../src/domain/play/progression/upgradeFallbackChoices.js');
