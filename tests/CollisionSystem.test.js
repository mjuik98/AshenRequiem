...
test('반경 내 적에게 hit 이벤트 발행', () => {
  ...
  sys.update({
    world: {
      entities: { player, enemies: [enemy], projectiles: [projectile], pickups: [] },
      queues: { events },
      presentation: { camera: { x: 0, y: 0, width: 1920, height: 1080 } },
    },
  });
...
test('반경 밖 적에게는 hit 이벤트 없음', () => {
  ...
  sys.update({
    world: {
      entities: { player, enemies: [enemy], projectiles: [projectile], pickups: [] },
      queues: { events },
      presentation: { camera: { x: 0, y: 0, width: 100, height: 100 } },
    },
  });
...
test('pierce=1인 투사체는 첫 번째 적 이후 관통 중단', () => {
  ...
  sys.update({
    world: {
      entities: { player, enemies: [e1, e2], projectiles: [proj], pickups: [] },
      queues: { events },
      presentation: { camera: { x: 0, y: 0, width: 1920, height: 1080 } },
    },
  });
...
test('CollisionSystem은 히트 이벤트만 예약하고 projectile 상태는 직접 소비하지 않는다', () => {
  ...
  sys.update({
    world: {
      entities: { player, enemies: [enemy], projectiles: [proj], pickups: [] },
      queues: { events },
      presentation: { camera: { x: 0, y: 0, width: 1920, height: 1080 } },
    },
  });
