...
test('Codex 기록 helper는 업적 진행도와 도감 발견 비율을 계산한다', () => {
  ...
  assert.equal(model.focusGoals.some((entry) => entry.kind === 'meta_upgrade' || entry.kind === 'daily' || entry.kind === 'codex'), true, '기록 탭 focus goals가 장기 메타 목표를 포함하지 않음');
  assert.equal(model.discoveryFocus.length === 3, true, '기록 탭 discovery focus가 적/무기/장신구 3종을 제공하지 않음');
  assert.equal(model.analytics.stageRecords.length > 0, true, '기록 탭 analytics에 stageRecords가 없음');
  assert.equal(model.analytics.deathCauseSummary.length > 0, true, '기록 탭 analytics에 deathCauseSummary가 없음');
  assert.equal(model.analytics.favoriteLoadout.archetypeId, 'spellblade', '기록 탭 analytics가 선호 archetype을 계산하지 않음');
  assert.notEqual(model.favoriteLoadout.weaponName, 'magic_bolt', '기록 탭 favorite loadout가 raw id를 그대로 유지하면 안 됨');
  assert.equal(model.favoriteLoadout.weaponName, 'Magic Bolt', '기록 탭 favorite weapon 이름 매핑이 없음');
  assert.equal(model.favoriteLoadout.archetypeName, 'Spellblade', '기록 탭 favorite archetype 이름 매핑이 없음');
});
