...
test('Codex 기록 helper는 업적 진행도와 도감 발견 비율을 계산한다', () => {
  ...
  assert.equal('favoriteLoadout' in model, false, '제거된 favorite loadout 프레젠테이션 필드가 남아 있으면 안 됨');
  assert.equal('recommendations' in model, false, '제거된 recommendation 프레젠테이션 필드가 남아 있으면 안 됨');
  assert.equal('recentRuns' in model, false, '제거된 recentRuns 프레젠테이션 필드가 남아 있으면 안 됨');
});
