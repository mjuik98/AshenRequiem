...
test('공통 서브스크린 토큰은 복귀 라벨과 공유 CSS를 제공한다', () => {
  ...
  assert.match(SUBSCREEN_SHARED_CSS, /scrollbar-color:\s*rgba\(232,\s*237,\s*245,\s*0\.34\)\s+transparent/);
  assert.match(SUBSCREEN_SHARED_CSS, /\.ss-scroll::-webkit-scrollbar\s*\{\s*width:\s*10px;/);
  assert.match(SUBSCREEN_SHARED_CSS, /border:\s*2px solid transparent/);
});
