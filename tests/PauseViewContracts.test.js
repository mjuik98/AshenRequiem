...
test('PauseView 스타일은 통합 로드아웃 카드와 반응형 마스터-디테일 레이아웃을 포함한다', () => {
  ...
  ['.pv-loadout-detail-hero', '.pv-loadout-detail-icon', '.pv-loadout-detail-tags', '.pv-loadout-detail-tag', '.pv-loadout-guidance', '.pv-loadout-row-label', '.pv-loadout-row-value', '.variant-status', '.variant-links', '.variant-synergy', '.variant-evolution', '.variant-guidance']
    .forEach((className) => {
      assert.equal(
        PAUSE_VIEW_CSS.includes(className),
        true,
