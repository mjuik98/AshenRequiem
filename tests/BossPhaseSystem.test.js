 ...
 *
 * CHANGE(P1-②): BossPhaseSystem 테스트 최초 추가
 *   - _phaseFlags 초기화 검증
 *   - HP 임계값 트리거 정확도
 *   - 동일 페이즈 중복 발동 방지
 *   - 비보스 엔티티 무시
 *   - bossData 없을 때 안전한 스킵
 *
 * 실행: npm test
 */
...
test('isBoss=false 적은 무시', () => {
  ...
  const enemy  = makeEnemy({ isBoss: false, hp: 100, maxHp: 1000 }); // 10% HP
  const events = makeEvents();
...
test('pendingDestroy 보스는 무시', () => {
  if (!BossPhaseSystem) return;
  const boss   = makeEnemy({ hp: 100, maxHp: 1000, pendingDestroy: true });
  const events = makeEvents();
...
test('isAlive=false 보스는 무시', () => {
  if (!BossPhaseSystem) return;
  const boss   = makeEnemy({ hp: 100, maxHp: 1000, isAlive: false });
  const events = makeEvents();
  run(boss, makeBossData(), events);
  assert.equal(events.bossPhaseChanged.length, 0);
});
...
test('HP 71% → 페이즈 이벤트 없음 (70% 임계값 미도달)', () => {
  if (!BossPhaseSystem) return;
  const boss   = makeEnemy({ hp: 710, maxHp: 1000 });
  const events = makeEvents();
  run(boss, makeBossData(), events);
  assert.equal(events.bossPhaseChanged.length, 0);
});
...
test('HP 70% 정확히 → 1페이즈 트리거', () => {
  if (!BossPhaseSystem) return;
  const boss   = makeEnemy({ hp: 700, maxHp: 1000 });
  const events = makeEvents();
  run(boss, makeBossData(), events);
...
test('HP 10% → 3페이즈 모두 트리거 (초기화 직후)', () => {
  if (!BossPhaseSystem) return;
  const boss   = makeEnemy({ hp: 100, maxHp: 1000 }); // 10% — 모든 임계값 통과
  const events = makeEvents();
  run(boss, makeBossData(), events);
...
test('동일 페이즈는 2회 호출에서 1번만 발행', () => {
  ...
  run(boss, makeBossData(), events);
  const firstEvents = [...events.bossPhaseChanged];
  ...
  assert.equal(firstEvents.length, 1, '첫 번째 발행이 1개가 아님');
});

test('_phaseFlags 첫 update에서 초기화됨', () => {
  if (!BossPhaseSystem) return;
  const boss   = makeEnemy({ hp: 1000, maxHp: 1000 }); // 100% HP
  const events = makeEvents();
  run(boss, makeBossData(), events);
...
test('bossData에 없는 enemyId 보스는 페이즈 없음', () => {
  if (!BossPhaseSystem) return;
  const boss   = makeEnemy({ enemyId: 'unknown_boss', hp: 100, maxHp: 1000 });
  const events = makeEvents();
