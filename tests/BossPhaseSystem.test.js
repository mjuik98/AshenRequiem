...
test('HP가 임계값 이하로 내려가면 bossPhaseChanged 이벤트가 발행된다', () => {
  ...
  const world = makeWorld({ entities: { enemies: [boss] } });

  BossPhaseSystem.update({ world, data });

  assert.equal(world.queues.events.bossPhaseChanged.length, 1, '페이즈 알림 미발행');
  assert.equal(world.queues.events.bossPhaseChanged[0].phaseIndex, 0, '잘못된 페이즈 인덱스');
...
test('이미 발생한 페이즈는 중복 발행되지 않는다', () => {
  ...
  assert.equal(world.queues.events.bossPhaseChanged.length, 0, '중복 발동 버그');
});
...
test('여러 페이즈를 한 번에 통과하면 여러 이벤트가 발행된다', () => {
  ...
  assert.equal(world.queues.events.bossPhaseChanged.length, 2, '다중 페이즈 발동 실패');
});

test('보스가 아닌 적은 스킵한다', () => {
  const normal = makeBoss({ isBoss: false, hp: 10, maxHp: 1000 });
  const data   = { bossData: makeBossData() };
  const world  = makeWorld({ entities: { enemies: [normal] } });

  BossPhaseSystem.update({ world, data });

  assert.equal(world.queues.events.bossPhaseChanged.length, 0, '일반 적이 페이즈 전환됨');
});
