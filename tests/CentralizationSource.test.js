...
test('씬과 UI는 공통 세션 옵션 모듈을 사용한다', () => {
  ...
  assert.equal(playSceneSource.includes('getEffectiveDevicePixelRatio'), true, 'PlayScene이 공통 DPR 헬퍼를 사용하지 않음');
  ...
  assert.equal(pauseViewSource.includes('SESSION_OPTION_DEFAULTS'), true, 'PauseView가 공통 옵션 기본값을 사용하지 않음');
});
