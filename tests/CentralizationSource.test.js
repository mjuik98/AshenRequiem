...
const keyboardAdapterSource = readProjectSource('../src/input/KeyboardAdapter.js');
const gamepadAdapterSource = readProjectSource('../src/input/GamepadAdapter.js');
const touchHudRuntimeSource = readProjectSource('../src/input/touchHudRuntime.js');
const touchAdapterOwnerSource = readProjectSource('../src/platform/browser/input/TouchAdapter.js');
const touchHudRuntimeOwnerSource = readProjectSource('../src/platform/browser/input/touchHudRuntime.js');
...
test('browser env와 session/data facade는 shared helper를 단일 소스로 사용한다', () => {
  ...
  assert.equal(browserGameInputRuntimeSource.includes("from '../../platform/browser/input/KeyboardAdapter.js'"), true, 'browser-owned gameInputRuntime이 platform keyboard adapter owner를 사용하지 않음');
  assert.equal(browserGameInputRuntimeSource.includes("from '../../platform/browser/input/GamepadAdapter.js'"), true, 'browser-owned gameInputRuntime이 platform gamepad adapter owner를 사용하지 않음');
  assert.equal(browserGameInputRuntimeSource.includes("from '../../platform/browser/input/TouchAdapter.js'"), true, 'browser-owned gameInputRuntime이 platform touch adapter owner를 사용하지 않음');
  ...
  assert.equal(permanentUpgradeDataSource.includes("from '../domain/play/progression/permanentUpgradeApplicator.js'"), true, 'permanentUpgradeData facade가 play progression owner applicator를 사용하지 않음');
  assert.equal(playerSpawnAppSource.includes("from '../../domain/play/progression/permanentUpgradeApplicator.js'"), true, 'playerSpawnApplicationService가 play progression owner applicator를 사용하지 않음');
  assert.equal(sessionRepositoryPortSource.includes("from '../../adapters/browser/session/sessionRepository.js'"), true, 'sessionRepositoryPort가 browser repository owner를 연결하지 않음');
  assert.equal(sessionPersistenceSource.includes("from './sessionRepositoryPort.js'"), true, 'sessionPersistenceService가 local repository port를 사용하지 않음');
  assert.equal(sessionSnapshotQuerySource.includes("from './sessionRepositoryPort.js'"), true, 'sessionSnapshotQueryService가 local repository port를 사용하지 않음');
  assert.equal(sessionSnapshotCommandSource.includes("from './sessionRepositoryPort.js'"), true, 'sessionSnapshotCommandService가 local repository port를 사용하지 않음');
...
test('platform browser input adapter는 owner path로 이동하고 기존 input 경로는 wrapper만 유지한다', () => {
  assert.equal(keyboardAdapterSource.includes("from '../platform/browser/input/KeyboardAdapter.js'"), true, 'KeyboardAdapter wrapper가 platform owner를 재노출하지 않음');
  assert.equal(gamepadAdapterSource.includes("from '../platform/browser/input/GamepadAdapter.js'"), true, 'GamepadAdapter wrapper가 platform owner를 재노출하지 않음');
  assert.equal(touchAdapterSource.includes("from '../platform/browser/input/TouchAdapter.js'"), true, 'TouchAdapter wrapper가 platform owner를 재노출하지 않음');
  assert.equal(touchHudRuntimeSource.includes("from '../platform/browser/input/touchHudRuntime.js'"), true, 'touchHudRuntime wrapper가 platform owner를 재노출하지 않음');
  assert.equal(touchAdapterOwnerSource.includes("from './touchHudRuntime.js'"), true, 'platform-owned TouchAdapter가 platform touch HUD helper를 사용하지 않음');
  assert.equal(touchHudRuntimeOwnerSource.includes('export function createTouchHudRuntime'), true, 'platform-owned touch HUD helper가 실구현을 소유하지 않음');
});
