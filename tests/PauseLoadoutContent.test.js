...
function makeTestFixture() {
  ...
  const synergy = {
    ...
    requires: ['up_magic_bolt', 'acc_iron_heart'],
    bonus: { damageDelta: 1 },
  };
...
test('buildPauseLoadoutItems는 무기, 장신구, 빈 슬롯을 포함하고 잠금 슬롯은 노출하지 않는다', () => {
  ...
  assert.equal(emptyItem.slotCategory, 'weapon', '첫 빈 슬롯이 무기 카테고리를 보존하지 않음');
  assert.equal(
    items.some((item) => item.kind === 'empty' && item.slotCategory === 'accessory'),
    true,
    '빈 장신구 슬롯이 slotCategory를 보존하지 않음',
  );
  assert.deepEqual(
...
test('getDefaultPauseSelection은 첫 무기를 우선하고 빈 로드아웃에서는 안정적이다', () => {
  ...
  assert.equal(emptyFirst.slotCategory, 'weapon', '빈 로드아웃 기본 선택이 슬롯 카테고리를 잃음');
});

test('renderPauseLoadoutPanel은 중복 요약 없이 핵심 상세 정보와 레벨 진행만 유지하며 긴 쿨다운 소수는 정리한다', () => {
  const api = getPauseLoadoutContent();
  ...
  assert.equal(html.includes('pv-loadout-overview'), false, '로드아웃 요약 밴드가 여전히 남아 있음');
  assert.equal(html.includes('활성 시너지 1'), false, '제거된 로드아웃 요약 텍스트가 여전히 남아 있음');
  assert.equal(html.includes('진화 준비 1'), false, '제거된 파워 스파이크 요약 텍스트가 여전히 남아 있음');
  ...
  assert.ok(html.includes('pv-slot-desc'), '슬롯 카드 보조 설명 줄이 없음');
  ...
  assert.ok(html.includes('variant-status'), '레벨 진행 섹션 variant 클래스가 없음');
  ...
  assert.equal(html.includes('pv-loadout-guidance'), false, '제거된 파워 스파이크 가이던스 블록이 여전히 남아 있음');
  assert.equal(html.includes('variant-guidance'), false, '제거된 가이던스 variant 클래스가 여전히 남아 있음');
  assert.ok(html.includes('pv-loadout-lv-block'), '레벨 진행 블록이 상세 패널에 포함되지 않음');
  assert.equal(html.includes('pv-loadout-progress-block'), false, '레벨 진행 섹션의 중첩 래퍼가 제거되지 않음');
  ...
  assert.ok(html.includes('pv-loadout-detail-tags'), '선택 항목 상태 배지 묶음이 없음');
  assert.ok(html.includes('pv-loadout-detail-tag'), '선택 항목 상태 배지가 없음');
  assert.ok(html.includes('detail-kind-weapon'), '무기 선택 accent 클래스가 상세 헤더에 없음');
  assert.ok(html.includes('사거리'), '무기 스탯에 사거리 항목이 없음');
  assert.ok(html.includes('범위'), '무기 스탯에 범위 항목이 없음');
  assert.ok(html.includes('투사체 수'), '무기 스탯에 투사체 수 항목이 없음');
  assert.ok(html.includes('발사체 속도'), '무기 스탯에 발사체 속도 항목이 없음');
  ...
  assert.equal(html.includes('현재 위력'), false, '중복 현재 위력 행이 여전히 남아 있음');
  assert.equal(html.includes('현재 쿨다운'), false, '중복 현재 쿨다운 행이 여전히 남아 있음');
  assert.equal(html.includes('연결 수'), false, '중복 연결 수 행이 여전히 남아 있음');
  ...
  assert.ok(html.includes('Seeks the nearest enemy with a compact arcane bolt.'), '슬롯 카드/상세 패널에 설명 기반 요약이 없음');
  ...
  assert.ok(html.includes('시너지 활성'), '선택 항목 상태 배지가 활성 시너지를 표시하지 않음');
  assert.ok(html.includes('진화 준비'), '선택 항목 상태 배지가 진화 준비를 표시하지 않음');
  ...
  assert.ok(html.includes('레벨 진행'), '무기 상세 패널 레벨 진행 제목이 없음');
  assert.ok(html.includes('Seeks the nearest enemy with a compact arcane bolt.'), '무기 상세 패널 요약이 설명 기반으로 렌더되지 않음');
  assert.equal(html.includes('다음 파워 스파이크'), false, '제거된 파워 스파이크 가이드 제목이 여전히 남아 있음');
  ...
  assert.equal(html.includes('현재 상태'), false, '이전 현재 상태 제목이 여전히 남아 있음');
  ...
  assert.ok(htmlWithAccessorySelected.includes('희귀'), '장신구 선택 상태 배지가 희귀도를 표시하지 않음');
  ...
  assert.ok(htmlWithAccessorySelected.includes('레벨 진행'), '장신구 상세 패널 레벨 진행 제목이 없음');
  assert.equal(htmlWithAccessorySelected.includes('다음 파워 스파이크'), false, '장신구 상세 패널에 제거된 파워 스파이크 가이드 제목이 남아 있음');
  assert.equal(htmlWithAccessorySelected.includes('현재 상태'), false, '장신구 상세 패널에 이전 현재 상태 제목이 남아 있음');
...
test('장신구 상세는 레벨별 나열 대신 현재 효과 요약만 표시한다', () => {
  const api = getPauseLoadoutContent();
  const player = makePlayer({
    weapons: [],
    accessories: [{
  ...
  const indexes = {
    weaponById: new Map(),
    accessoryById: new Map([['duplicator', data.accessoryData[0]]]),
    synergiesByWeaponId: new Map(),
    synergiesByAccessoryId: new Map(),
  };

  const html = api.renderPauseLoadoutPanel({
    items: api.buildPauseLoadoutItems({ player }),
    selectedItemKey: 'accessory:0',
    player,
    data,
    indexes,
  });

  assert.equal(html.includes('레벨별 효과'), false, '장신구 상세에 레벨별 효과 섹션이 남아 있음');
  assert.equal(html.includes('효과'), true, '장신구 상세에 효과 섹션 제목이 없음');
  assert.equal(html.includes('추가 투사체 +3발'), true, '장신구 상세가 현재 레벨 기준 효과를 보여주지 않음');
  assert.equal(html.includes('Lv1 +1발'), false, '장신구 상세가 레벨별 나열 텍스트를 여전히 출력함');
});

test('연결 아이템은 진화 재료 전용 항목을 반복 노출하지 않고 진화 섹션이 전담한다', () => {
  const api = getPauseLoadoutContent();
  const lightningRing = makeWeapon({
    id: 'lightning_ring',
    name: '번개의 고리',
    behaviorId: 'orbit',
    icon: '⚡',
  ...
  const data = {
    weaponData: [lightningRing],
    accessoryData: [{ id: 'scattered_shot', name: '산탄 마법진', icon: '✹', rarity: 'rare', maxLevel: 5 }],
    synergyData: [],
    weaponEvolutionData: [{
  ...
  const indexes = {
    weaponById: new Map([[lightningRing.id, lightningRing]]),
    accessoryById: new Map([['scattered_shot', { id: 'scattered_shot', name: '산탄 마법진', icon: '✹', rarity: 'rare', maxLevel: 5 }]]),
    synergiesByWeaponId: new Map(),
    synergiesByAccessoryId: new Map(),
  };
  const html = api.renderPauseLoadoutPanel({
  ...
  const linkedItemsSection = extractSection(html, 'pv-loadout-linked-items');
  const evolutionSection = extractSection(html, 'pv-loadout-evolution');
  const accessoryMentionCount = (evolutionSection.match(/산탄 마법진/g) ?? []).length;

  assert.equal(linkedItemsSection.includes('산탄 마법진'), false, '진화 재료가 연결 아이템에도 반복 노출됨');
  assert.equal(evolutionSection.includes('산탄 마법진'), true, '진화 섹션이 진화 재료를 보여주지 않음');
  assert.equal(accessoryMentionCount, 1, '진화 섹션이 같은 재료 이름을 중복 노출함');
});
