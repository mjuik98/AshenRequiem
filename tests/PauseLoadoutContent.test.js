...
test('renderPauseLoadoutPanel은 중복 요약 없이 핵심 상세 정보와 레벨 진행만 유지하며 긴 쿨다운 소수는 정리한다', () => {
  ...
  assert.ok(html.includes('pv-loadout-guidance'), '다음 파워 스파이크 가이던스 블록이 없음');
  ...
  assert.ok(html.includes('variant-guidance'), '가이던스 섹션 variant 클래스가 없음');
  ...
  assert.ok(html.includes('다음 파워 스파이크'), '무기 상세 패널에 다음 파워 스파이크 가이드 제목이 없음');
  ...
  assert.ok(htmlWithAccessorySelected.includes('다음 파워 스파이크'), '장신구 상세 패널에 다음 파워 스파이크 가이드 제목이 없음');
