...
await test('core smoke wrapper는 로컬 suite 완료를 위한 여유 timeout budget을 둔다', async () => {
  ...
  assert.equal(
    smokeWrapperSource.includes('const SMOKE_TIMEOUT_MS = 300_000;'),
    true,
    'core smoke timeout budget이 확장된 suite를 감당할 만큼 충분히 커야 함',
  );
});
...
await test('browser smoke scenario registry는 title codex/settings 흐름을 포함한다', async () => {
  ...
  assert.equal(typeof SCENARIOS.title_loadout_accessibility, 'object', 'title_loadout_accessibility 시나리오가 등록되지 않음');
  ...
  assert.equal(ids.includes('title_loadout_accessibility'), true);
  ...
  assert.equal(SCENARIOS.title_loadout_accessibility.suite, 'core', 'title_loadout_accessibility가 core smoke suite에 속해야 함');
  ...
  assert.equal(SCENARIOS.title_loadout_accessibility.stepNames.includes('responsive'), true, 'title_loadout_accessibility 시나리오가 responsive 검증을 포함하지 않음');
  assert.equal(SCENARIOS.title_loadout_accessibility.stepNames.includes('keyboard'), true, 'title_loadout_accessibility 시나리오가 keyboard 검증을 포함하지 않음');
  assert.equal(SCENARIOS.pause_overlay.stepNames.includes('keyboard'), true, 'pause_overlay 시나리오가 keyboard 검증을 포함하지 않음');
  assert.equal(SCENARIOS.levelup_overlay.stepNames.includes('keyboard'), true, 'levelup_overlay 시나리오가 keyboard 검증을 포함하지 않음');
  assert.equal(SCENARIOS.result_screen.stepNames.includes('keyboard'), true, 'result_screen 시나리오가 keyboard 검증을 포함하지 않음');
  ...
  assert.equal(resolveScenarioArtifactDir('title_loadout_accessibility'), 'output/web-game/deterministic-smoke-core/title-loadout-accessibility');
...
await test('preview smoke wrapper는 prebuilt dist가 있으면 build 단계를 건너뛸 수 있다', async () => {
  ...
  await smokeWrapper.runSmokeAgainstPreview(
    { host: '127.0.0.1', port: 4173, suite: 'core', all: false, skipBuild: true },
    {
      ...
      spawnCommandFn: () => {
        lifecycle.push('preview:spawned');
        return new EventEmitter();
      },
      startStaticDistServerFn: async ({ port }) => {
        lifecycle.push(`static:${port}`);
        return { close: async () => lifecycle.push('static:closed') };
      },
      waitForPreviewReadyFn: async () => {
        lifecycle.push('preview:waited');
      },
      stopChildProcessFn: async () => lifecycle.push('preview:stopped'),
    },
  );
  ...
  assert.deepEqual(lifecycle, ['static:4173', 'static:closed'], 'skipBuild=true이면 정적 dist 서버를 바로 사용해야 함');
});

await test('preview smoke wrapper는 preview 준비 실패 시 정적 dist 서버로 fallback한다', async () => {
  const smokeWrapper = await import('../scripts/browser-smoke/runSmokeAgainstPreview.mjs');
  const commandCalls = [];
  const lifecycle = [];
  const preview = new EventEmitter();
  ...
  await smokeWrapper.runSmokeAgainstPreview(
    { host: '127.0.0.1', port: 4173, scenario: 'pause_overlay', all: false, skipBuild: false },
    {
      findAvailablePortFn: async () => 4173,
      runCommandFn: async (command, args) => {
        commandCalls.push([command, args]);
      ...
      waitForPreviewReadyFn: async () => {
        throw new Error('preview bootstrap failed');
      },
      startStaticDistServerFn: async ({ port }) => {
        lifecycle.push(`fallback:${port}`);
        return { close: async () => lifecycle.push('fallback:closed') };
      },
      stopChildProcessFn: async (child) => {
        lifecycle.push(`preview:stopped:${child.pid}`);
      },
    },
  );
  ...
  assert.deepEqual(lifecycle, [
    ...
    'fallback:4173',
    'fallback:closed',
  ], 'preview 실패 시 fallback 서버 lifecycle이 기대와 다름');
  assert.equal(commandCalls.length, 2, 'fallback 경로는 build 후 smoke runner까지 실행해야 함');
  assert.equal(commandCalls[1][1].includes('--scenario'), true, 'fallback smoke가 대상 시나리오를 유지하지 않음');
});
