...
test('pipeline builder는 문서 snapshot 대신 runtime manifest를 직접 사용한다', () => {
  ...
  assert.equal(
    pipelineBuilderSource.includes('PLAY_PIPELINE_INSTANCE_SYSTEMS'),
    true,
    'PipelineBuilder가 registry instance manifest를 직접 순회해야 함',
  );
  assert.equal(
    pipelineBuilderSource.includes('pipeline.register(this._eventRegistry.asSystem(), { priority: 105 })'),
    false,
    'PipelineBuilder가 registry instance priority를 하드코딩하면 안 됨',
  );
});
