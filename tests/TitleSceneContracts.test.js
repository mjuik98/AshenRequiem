...
test('TitleScene 종료 버튼은 활성 상태이며 종료 실패 시 상태 helper가 안내 문구를 갱신한다', () => {
  ...
  assert.equal(
    TITLE_SCREEN_HTML.includes('data-action="continue"'),
    false,
    '타이틀 화면에 제거된 Continue 액션이 아직 남아 있음',
  );
...
test('타이틀 하위 화면 runtime view는 공통 ss-root와 메인 화면 복귀 라벨을 사용한다', () => {
  ...
  try {
    ...
    assert.equal(metaShop.el.innerHTML.includes('ms-panel ss-panel ss-scroll'), true, 'MetaShopView 패널이 공통 스크롤바 클래스를 사용하지 않음');
