...
test('burst draw 함수는 sprite draw가 false면 기존 vector path로 폴백한다', () => {
  ...
  fn(
    ctx,
    makeEffect({ effectType: 'burst', effectVisualId: 'burst', lifetime: 0.1, maxLifetime: 0.45 }),
    camera,
    1,
    { drawEffectSprite: () => false },
  );

  assert.equal(calls.some((call) => call.fn === 'arc'), false, 'cataloged burst는 sprite 미준비 시 vector fallback을 그리면 안 됨');
});
...
test('non-cataloged burst effect는 기존 vector path를 유지한다', () => {
  const { ctx, calls } = makeCtx();
  const fn = getEffectDraw('burst');

  fn(
    ctx,
    makeEffect({ effectType: 'burst', effectVisualId: 'unknown_burst_visual', lifetime: 0.1, maxLifetime: 0.45 }),
    camera,
    1,
    { drawEffectSprite: () => false },
  );

  assert.equal(calls.some((call) => call.fn === 'arc'), true, 'non-cataloged burst는 vector path가 유지되어야 함');
});
