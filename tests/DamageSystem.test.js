...
test('hits 이벤트로 적의 hp가 감소한다', () => {
  ...
  const world  = makeWorld({ events: makeEvents({ hits: [hit] }) });
...
test('hp가 0 이하가 되면 isAlive=false가 되고 deaths 큐에 추가된다', () => {
  ...
  const world = makeWorld({ enemies: [enemy], events: makeEvents({ hits: [hit] }) });
  DamageSystem.update({ world });
  assert.equal(enemy.isAlive, false, '사망 후 isAlive가 여전히 true');
  assert.ok(world.events.deaths.some(d => d.entity === enemy || d.entityId === enemy.id),
    'deaths 큐에 사망 이벤트 없음');
});
