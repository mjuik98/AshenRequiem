...
test('확장 무기 9종은 전용 진화 레시피를 가진다', () => {
  const expected = [
    ...
    ['holy_bolt', 'holy_bolt_upgrade'],
    ['ice_bolt', 'ice_bolt_upgrade'],
  ];
...
test('holy/ice bolt 진화 레시피는 지정된 accessory 계약을 유지한다', () => {
  const holyRecipe = weaponEvolutionData.find((item) => item.requires.weaponId === 'holy_bolt');
  const iceRecipe = weaponEvolutionData.find((item) => item.requires.weaponId === 'ice_bolt');

  assert.ok(holyRecipe, 'holy_bolt 진화 레시피가 없음');
  assert.ok(iceRecipe, 'ice_bolt 진화 레시피가 없음');
  assert.deepEqual(holyRecipe.requires.accessoryIds, ['swift_hourglass'], 'holy_bolt 진화 accessory 계약 불일치');
  assert.deepEqual(iceRecipe.requires.accessoryIds, ['crystal_lens'], 'ice_bolt 진화 accessory 계약 불일치');
});
