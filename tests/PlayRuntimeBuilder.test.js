...
test('buildPlayRuntime는 플레이 런타임 조립을 한 곳에서 수행한다', () => {
  ...
  const registerEventHandlersImpl = () => {};
...
test('bootstrapPlaySceneRuntime는 전용 runtime builder에 위임한다', () => {
  ...
  assert.equal(calls[0].runtimeServices, runtimeServices, 'bootstrap helper가 injected runtime services를 runtime builder에 전달하지 않음');
  assert.equal(calls[0].registerEventHandlersImpl, registerPlayEventHandlers, 'bootstrap helper가 injected event registration orchestrator를 runtime builder에 전달하지 않음');
});
