...
test('critChance=1.0이면 항상 크리티컬 발생 (데미지 배율 2배)', () => {
  ...
  const world  = makeWorld({ player, events: makeEvents({ hits: [hit] }) });
...
test('critMultiplier가 높을수록 크리 피해량 증가', () => {
  ...
  const world1  = makeWorld({ player: player1, events: makeEvents({ hits: [makeHitEvent({ target: enemy1, attackerId: 'player', damage: 10 })] }) });
  const world2  = makeWorld({ player: player2, events: makeEvents({ hits: [makeHitEvent({ target: enemy2, attackerId: 'player', damage: 10 })] }) });
