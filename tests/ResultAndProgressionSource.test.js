...
test('결과 화면은 사용 무기, Ascension, 획득 재화, 이번 런 해금만 남기고 나머지 섹션은 숨긴다', () => {
  const { document, restore } = installMockDom();
  ...
  try {
    ...
    assert.equal(view.el.innerHTML.includes('사용한 무기'), true, '사용 무기 섹션이 복원되지 않음');
    assert.equal(view.el.innerHTML.includes('Holy Aura'), true, '사용 무기 상세가 복원되지 않음');
    assert.equal(view.el.innerHTML.includes('Ascension'), true, 'Ascension 섹션이 복원되지 않음');
    assert.equal(view.el.innerHTML.includes('이번 런 획득 재화'), true, '획득 재화 섹션이 복원되지 않음');
    assert.equal(view.el.innerHTML.includes('+ 65 💰'), true, '획득 재화 상세가 복원되지 않음');
    assert.equal(view.el.innerHTML.includes('다음 목표'), false, '다음 목표 섹션이 남아 있음');
    assert.equal(view.el.innerHTML.includes('백전노장'), false, '다음 목표 항목이 여전히 렌더됨');
    assert.equal(view.el.innerHTML.includes('데일리 챌린지'), false, 'daily challenge 섹션이 남아 있음');
    assert.equal(view.el.innerHTML.includes('daily-2026-03-27'), false, 'daily reward 상세가 여전히 렌더됨');
    assert.equal(view.el.innerHTML.includes('추천 조정'), false, '추천 조정 섹션이 남아 있음');
    assert.equal(view.el.innerHTML.includes('Moon Crypt 회피'), false, '추천 조정 항목이 여전히 렌더됨');
    assert.equal(view.el.innerHTML.includes('전투 복기'), false, '전투 복기 섹션이 남아 있음');
    assert.equal(view.el.innerHTML.includes('망령 탄막에 밀려 전열이 무너졌습니다.'), false, 'death recap 상세가 여전히 렌더됨');
    assert.equal(view.el.innerHTML.includes('최근 런'), false, '최근 런 섹션이 남아 있음');
  } finally {
    restore();
  }
});
