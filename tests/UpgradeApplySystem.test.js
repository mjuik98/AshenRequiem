...
test('UpgradeApplySystem은 weapon_evolution 선택 시 기반 무기를 진화 무기로 교체한다', () => {
  ...
  assert.equal(world.player.weapons[0]?.id, 'arcane_nova', 'weapon_evolution이 기반 무기를 교체하지 않음');
  assert.equal(world.player.evolvedWeapons.has('evolution_arcane_nova'), true, '적용된 진화 레시피가 기록되지 않음');
  assert.equal(world.events.weaponEvolved[0]?.evolvedWeaponId, 'arcane_nova', '진화 선택 후 weaponEvolved 이벤트가 누락됨');
});
...
test('LevelSystem은 진화 조건 충족 시 weapon_evolution 선택지를 최우선으로 노출한다', () => {
  ...
  assert.equal(world.pendingLevelUpChoices[0]?.type, 'weapon_evolution', '진화 선택지가 최우선 노출되지 않음');
  assert.equal(world.pendingLevelUpChoices[0]?.resultWeaponId, 'arcane_nova', '진화 선택지 결과 무기가 잘못됨');
});
