...
try {
  audioRuntime = await import('../src/adapters/browser/audioRuntime.js');
} catch (error) {
  audioRuntime = { error };
}
...
function getAudioRuntimeApi() {
  assert.ok(
    !audioRuntime.error,
    audioRuntime.error?.message ?? 'src/adapters/browser/audioRuntime.js가 아직 없음',
  );
  return audioRuntime;
}
...
test('sound runtime helper modules expose graph and voice helpers', () => {
  ...
  const audioRuntimeApi = getAudioRuntimeApi();
  ...
  assert.equal(typeof audioRuntimeApi.createBrowserAudioContextFactory, 'function', 'browser audio runtime factory helper가 없음');
});
...
test('sound system state는 browser time/audio factory를 주입 가능한 상태로 보관한다', () => {
  const systemStateApi = getSoundSystemStateApi();
  const nowSeconds = () => 12.5;
  const createAudioContext = () => ({ state: 'running' });

  const state = systemStateApi.createSoundSystemState({
    sfxDefs: {},
    bgmDefs: {},
    nowSeconds,
    createAudioContext,
  ...
  assert.equal(state._nowSeconds, nowSeconds, 'sound state가 nowSeconds helper를 보관하지 않음');
  assert.equal(state._createAudioContext, createAudioContext, 'sound state가 audio context factory를 보관하지 않음');
});

test('sound lifecycle은 target에 주입된 audio context factory를 우선 사용한다', () => {
  const lifecycleApi = getSoundSystemLifecycleApi();
  ...
  lifecycleApi.initSoundSystemContext(target, {
    createGraph: () => ({
      masterBus: {},
      bgmBus: {},
      sfxBus: {},
  ...
  assert.equal(created, 1, 'sound lifecycle이 target 주입 factory를 사용하지 않음');
});
