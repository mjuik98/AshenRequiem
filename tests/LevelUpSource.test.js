...
test('level up controller는 런 guidance가 지정한 추천 빌드 카드에 전용 priority hint를 붙인다', () => {
  const world = makeWorld({
    entities: { player: makePlayer({
      weapons: [{ id: 'magic_bolt', level: 7, currentCooldown: 0 }],
      accessories: [],
  ...
  assert.equal(shownConfigs[0].choices[0].priorityHint, '추천 빌드 경로', '추천 빌드 카드에 guidance priority hint가 없음');
  assert.equal(shownConfigs[0].choices[0].priorityHintType, 'recommended', '추천 빌드 카드가 recommended tone을 사용하지 않음');
});
