...
const hudMarkupSource = readProjectSource('../src/ui/hud/hudViewMarkup.js');

test('HUD는 markup/style helper를 통해 구조와 스타일을 분리한다', () => {
  assert.equal(hudSource.includes("from './hudViewMarkup.js'"), true, 'HudView가 markup helper를 사용하지 않음');
  assert.equal(hudSource.includes("from './hudViewStyles.js'"), true, 'HudView가 style helper를 사용하지 않음');
});

test('HUD는 시간/킬과 함께 이번 런 골드 영역을 노출한다', () => {
  assert.equal(hudMarkupSource.includes('hud-gold'), true, 'HUD 골드 슬롯 클래스가 없음');
...
test('HUD는 현재 저주 수치를 별도 영역으로 노출한다', () => {
  assert.equal(hudMarkupSource.includes('hud-curse'), true, 'HUD 저주 슬롯 클래스가 없음');
