...
test('CodexView는 장신구 도감 탭과 발견 수 집계를 포함한다', () => {
  assert.equal(codexViewSource.includes('data-tab="accessory"'), true, 'CodexView에 장신구 도감 탭이 없음');
  assert.equal(codexViewSource.includes('cx-tab-accessory'), true, 'CodexView에 장신구 패널이 없음');
  assert.equal(codexViewSource.includes('cx-discovery-strip'), true, 'CodexView에 분리 발견 통계 strip이 없음');
...
test('CodexView 장신구 도감은 검색/필터/상세 패널 구조를 가진다', () => {
  assert.equal(codexViewSource.includes('_accessorySearch'), true, 'CodexView에 장신구 검색 상태가 없음');
  assert.equal(codexViewSource.includes('_accessoryRarityFilter'), true, 'CodexView에 장신구 rarity 필터 상태가 없음');
  assert.equal(codexViewSource.includes('_accessoryEffectFilter'), true, 'CodexView에 장신구 effect 필터 상태가 없음');
  assert.equal(codexViewSource.includes('cx-accessory-detail'), true, 'CodexView에 장신구 상세 패널이 없음');
  assert.equal(codexViewSource.includes('cx-discovery-hint'), true, '미발견 장신구 힌트가 없음');
});
...
test('CodexView 무기 도감은 진화 재료 장신구 클릭으로 장신구 도감 점프를 지원한다', () => {
  ...
  assert.equal(codexViewSource.includes('data-accessory-ref'), true, '무기 도감 진화 재료 칩 훅이 없음');
});
