...
test('메타/도감 view-model 계산은 stable presentation builders로 분리되고 app 계층은 compatibility facade를 유지한다', () => {
  assert.ok(
    !metaShopViewModelService?.error,
  ...
  const metaShopServiceSource = readProjectSource('../src/app/meta/metaShopViewModelService.js');
  const codexRecordsServiceSource = readProjectSource('../src/app/meta/codexRecordsQueryService.js');

  assert.equal(
    metaShopViewSource.includes("from '../../app/meta/metaShopViewModelService.js'"),
    false,
    'MetaShopView가 app-level view model service에 직접 결합되면 안 됨',
  );
  assert.equal(
    metaShopViewSource.includes("from './metaShopModel.js'"),
    true,
    'MetaShopView가 local presentation entrypoint를 사용하지 않음',
  );
  assert.equal(
    metaShopModelSource.includes("from '../../domain/meta/metashop/metaShopPresentation.js'"),
    true,
    'metaShopModel helper가 stable presentation module을 사용하지 않음',
  );
  assert.equal(
    metaShopServiceSource.includes("from '../../domain/meta/metashop/metaShopPresentation.js'"),
    true,
    'metaShopViewModelService가 compatibility facade로 stable presentation module을 재노출하지 않음',
  );
  assert.equal(
    codexRecordsSource.includes("from '../../domain/meta/codex/codexRecordsPresentation.js'"),
    true,
    'codexRecords helper가 stable presentation module을 사용하지 않음',
  );
  assert.equal(
    codexRecordsTabSource.includes("from './codexRecords.js'"),
    true,
    'codex records tab가 local presentation entrypoint를 사용하지 않음',
  );
  assert.equal(
    codexRecordsServiceSource.includes("from '../../domain/meta/codex/codexRecordsPresentation.js'"),
    true,
    'codexRecordsQueryService가 compatibility facade로 stable presentation module을 재노출하지 않음',
  );
});
