 ...
 *
 * CHANGE (Phase 4): 신규 능력치 stat 처리 추가
 *   - cooldownMult       : Math.max(0.1, current + value) 로 클램핑
 *   - projectileSpeedMult: flat 가산
 *   - projectileSizeMult : flat 가산
 *   - xpMult             : flat 가산
 *   - _applyAccessoryEffects: 신규 stat 케이스 추가
 */
  ...
  applyUpgrade(player, upgrade, synergyData, synergyState) {
    ...
    if (upgrade.type === 'weapon_new') {
      ...
      if (def) {
        ...
        // globalDamageMult 반영 (장신구/영구강화 누산 배율)
      ...
      if (eff) {
        if (eff.stat === 'hp') {
          ...
          // 쿨다운 배율은 최솟값 0.1(90% 단축)으로 클램핑
          player.cooldownMult = Math.max(0.1,
            (player.cooldownMult ?? 1.0) + eff.value);

        } else if (player[eff.stat] !== undefined) {
          // projectileSpeedMult, projectileSizeMult, xpMult, moveSpeed,
          // magnetRadius, lifesteal 등 단순 가산 스탯
          player[eff.stat] += eff.value;

        } else {
          // 플레이어에 해당 필드가 없으면 초기화 후 적용
          player[eff.stat] = eff.value;
        }
      }
    }
 ...
 *
 * CHANGE (Phase 4): 신규 능력치 케이스 추가
 *   cooldownMult, projectileSpeedMult, projectileSizeMult, xpMult
 */
function _applyAccessoryEffects(player, effects) {
  for (const eff of effects) {
    switch (eff.stat) {
      ...
      // ── Phase 4: 신규 케이스 ──────────────────────────────────────
      case 'cooldownMult':
        // 장신구 쿨다운 감소 (최솟값 0.1 클램핑)
        player.cooldownMult = Math.max(0.1,
          (player.cooldownMult ?? 1.0) + eff.value);
        break;

      case 'projectileSpeedMult':
        player.projectileSpeedMult =
          (player.projectileSpeedMult ?? 1.0) + eff.value;
        break;

      case 'projectileSizeMult':
        player.projectileSizeMult =
          (player.projectileSizeMult ?? 1.0) + eff.value;
        break;
