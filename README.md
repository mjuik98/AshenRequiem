...
- Source code licensing is defined in [LICENSE](./LICENSE).
- Bundled art/media files are tracked separately in [ASSETS.md](./ASSETS.md).
- Security reporting guidance is documented in [SECURITY.md](./SECURITY.md).
- A release-owner checklist for private-to-public conversion lives in [docs/public-release-checklist.md](./docs/public-release-checklist.md).
...
docs/        아키텍처 현재 상태, wrapper inventory, module map, release checklist
docs/superpowers/  로컬 planning/spec scratch 문서용 경로 (git 추적 제외)
...
깨끗한 재현 기준선이 필요할 때는 `npm install` 대신 `npm ci`를 권장한다.
...
| `?profilePipeline` | 브라우저 query flag | 전역을 건드리지 않고 pipeline profiler를 활성화 |
| `?forceTouchHud` | 브라우저 query flag | 터치 HUD를 강제로 노출해 mobile smoke/QA를 재현 |
| `__ASHEN_RUNTIME_DEBUG__ = true \| 'scope' \| ['scope']` | browser global | runtime logger/debug surface의 scope별 디버그 출력을 활성화 |
| `__ASHEN_DEBUG_RUNTIME__ = true` | browser global | query flag 없이 runtime hooks를 강제로 활성화 |

## 공개 전환 전 확인 사항

- 현재 트리 기준 일반 개발/검증에는 `.env`가 필요하지 않다.
- 저장소를 public으로 전환하기 전에 bundled raster asset의 공개 재배포 가능 여부는 별도로 확인해야 한다.
- `progress.md`, `status.txt`, `docs/superpowers/*`는 로컬 scratch 경로로 gitignore 되어야 하며, force-add로 히스토리에 들어가지 않았는지 최종 확인하는 편이 안전하다.
